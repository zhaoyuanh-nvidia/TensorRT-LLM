...
class DSAtrtllmAttentionMetadata(TrtllmAttentionMetadata):
    ...
    def gen_token_repeats(self, device=None) -> torch.Tensor:
        return torch.tensor(
            self.gen_token_repeat_list(), dtype=torch.long, device=device
        )

    def expand_per_gen_token(
        self, values: torch.Tensor, dim: int = 0
    ...
    def on_update_kv_lens(self) -> None:
        ...
        if self.num_generations > 0:
            ...
            if self.use_expanded_buffers_for_mtp:
                kv_lens_expanded, num_tokens = self.expand_per_gen_token(
                    gen_indexer_kv_lens
                )
    ...
    def prepare_for_spec_decode(self, kv_lens: torch.Tensor):
        ...
        if not self.is_ragged_verify:
            ...
            assert not self.is_spec_decoding_enabled, (
                "ragged verification requires dense spec-decoding metadata "
                "to be disabled"
            )

        self.use_expanded_buffers_for_mtp = not use_dsl and (
            self.is_ragged_verify
            or (self.max_draft_tokens > 1 and get_sm_version() == 90)
            or (
    ...
    def _prepare_ragged_row_kv_lens(self, kv_lens: torch.Tensor) -> None:
        ...
        for request_idx, (kv_len, verify_len) in enumerate(
            zip(gen_kv_lens, verify_lens)
        ):
        ...
        self.row_kv_lens_host[:num_rows].copy_(
            torch.tensor(rows, dtype=torch.int32)
        )
        self.row_kv_lens_cuda[:num_rows].copy_(
            self.row_kv_lens_host[:num_rows], non_blocking=True
        )
        self.row_kv_correction_host[:num_rows].copy_(
            torch.tensor(corrections, dtype=torch.int32)
        )
        self.row_req_idx_host[:num_rows].copy_(
        ...
        self.row_req_idx_cuda[:num_rows].copy_(
            self.row_req_idx_host[:num_rows], non_blocking=True
        )
    ...
    def _prepare_token_major_gen_rows(self, kv_lens: torch.Tensor) -> None:
        ...
        assert num_rows <= capacity, (
            f"token-major generation needs {num_rows} rows but capacity is "
            f"{capacity}"
        )

        if num_contexts:
            self.attn_row_kv_lens_host[:num_contexts].copy_(
                kv_lens[:num_contexts].to(torch.int32)
            )
    ...
    def refresh_token_major_gen_rows(self) -> None:
        ...
        self.attn_row_kv_lens_cuda[:num_rows].add_(
            self.attn_row_kv_correction_cuda[:num_rows]
        )
    ...
    def apply_device_ragged_layout(
        ...
        self.seq_lens_cuda[num_contexts : self.num_seqs] = verify_lens.to(
        ...
        self.block_table_expanded[:num_rows, :width] = (
            self.indexer_k_cache_block_offsets[
                num_contexts : self.num_seqs, :width
            ]
    ...
    def _refresh_expanded_block_table(self, repeat_factor: Optional[int] = None):
        ...
        if use_runtime_layout:
            expanded_blocks, num_tokens = self.expand_per_gen_token(
                gen_block_tensor, dim=0
            )
        else:
