...
class DSv4DSparkWorker(SpecWorkerBase):
    ...
    def _forward_impl(
        ...
        # These device values ride the sampler's existing D2H batch.
        ...
        # ``verify_lens`` is generation-indexed, so publish it only on pure
        # generation steps where its rows align with the batch outputs.
        if (
