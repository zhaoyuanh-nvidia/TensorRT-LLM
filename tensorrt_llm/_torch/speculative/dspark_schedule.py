...
def _schedule_topk_rank_kernel(
    ...
    survival = tl.load(
        survival_ptr
        + request * stride_row
        + (position + FLOOR).to(tl.int64) * stride_position,
        mask=candidate_mask,
        other=0.0,
    ).to(tl.float32)
    ...
    for peer_start in range(0, NUM_CANDIDATES, BLOCK_CP):
        ...
        peer_valid = peer_mask & (peer_request < num_real) & (
            peer_survival >= survival_eps
        )
        peer_masked_survival = tl.where(
            peer_valid, peer_survival, float("-inf")
        )
        ...
        earlier = (peer_position[None, :] < position[:, None]) | (
            (peer_position[None, :] == position[:, None])
            & (peer_request[None, :] < request)
        )
...
@triton.jit(
    do_not_specialize=["num_real", "budget", "pad_len", "graph_num_tokens"]
)
...
def _schedule_verify_lens_topk_fused_fill_triton(
    *,
    ...
    if num_rows > 256:
        raise ValueError(
            f"fused top-k scheduler supports at most 256 rows, got {num_rows}"
        )
    schedulable = int(cfg.schedulable_per_request)
    ...
    if ranked:
        ...
        try:
            ...
            raise DSparkFusedScheduleError(
                "DSpark fused top-k rank launch failed"
            ) from exc
    ...
    try:
        ...
        raise DSparkFusedScheduleError(
            "DSpark fused round-robin fill launch failed"
        ) from exc
    return output
...
def schedule_verify_lens_topk_fused_fill(
    *,
    ...
    if survival.dim() != 2:
        raise ValueError(
            f"survival must be [bs, K], got shape {tuple(survival.shape)}"
        )
    num_rows, num_positions = survival.shape
    if num_positions != cfg.block_size:
        raise ValueError(
            f"survival has {num_positions} positions but block_size is "
            f"{cfg.block_size}"
        )
    budget = int(budget)
    ...
    if not 0 <= num_real <= num_rows:
        raise ValueError(
            f"num_real must be in [0, {num_rows}], got {num_real}"
        )
    max_token_len = int(cfg.resolved_max_verify_len) + 1
    if not 1 <= pad_len <= max_token_len:
        raise ValueError(
            f"pad_len must be in [1, {max_token_len}], got {pad_len}"
        )
    ...
    scheduled_capacity = min(
        max(budget, 0), num_real * int(cfg.schedulable_per_request)
    )
    ...
    if spare != 0:
        raise ValueError(
            f"fused schedule left {spare} graph tokens after round-robin fill"
        )
    return token_lens
