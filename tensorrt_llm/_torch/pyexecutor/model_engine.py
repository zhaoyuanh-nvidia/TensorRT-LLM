...
class PyTorchModelEngine(ModelEngine):
        ...
        with torch.inference_mode():
            for graph_bs in sorted(int(g) for g in exact_table.tables):
                ...
                try:
                    ...
                    graph_num_tokens = (
                        graph_bs * (int(cfg.min_verify_len) + 1)
                        + warmup_budget)
                    schedule_verify_lens_topk_fused_fill(
                        survival=survival,
                ...
                try:
                    ...
                    raise RuntimeError(
                        ...
                        f"CUDA gate for G={graph_bs}"
                    ) from exc
                ready.append(graph_bs)

        self._dspark_fused_schedule_ready_sizes = set(ready)
        logger.info(
            "DSpark fused scheduler prewarm complete for graph "
            f"batch sizes {ready}"
        )
    ...
    def _select_dspark_windows_with_fused_fallback(
        ...
        *,
        ...
        try:
            return select_fn(
                **selector_kwargs, use_fused_exact=use_fused_exact)
            ...
            failure_counts = getattr(
                self, "_dspark_fused_schedule_failure_counts", {})
