...
def test_production_policy_has_no_profiling_control_state() -> None:
    planner = _planner(device_windows=True, skip_mixed_trim=True)

    assert planner.device_windows
    assert planner.skip_mixed_trim
    assert not hasattr(planner, "detailed_stats")
    assert not any("forced" in name or "pending" in name for name in vars(planner))
