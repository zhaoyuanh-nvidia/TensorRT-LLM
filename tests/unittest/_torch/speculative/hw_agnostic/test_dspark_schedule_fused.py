...
def _legacy_schedule_and_fill(
    *,
    ...
    scheduled = schedule_verify_lens_topk(
        survival=real_survival, budget=budget, cfg=cfg)
...
@pytest.mark.parametrize("num_rows", [1, 2, 4, 8])
@pytest.mark.parametrize("num_real", [0, 1, 2, 4])
@pytest.mark.parametrize("budget", [0, 1, 3, 9])
@pytest.mark.parametrize("survival_eps", [1e-6, 0.35])
def test_cpu_fused_helper_matches_established_schedule_and_fill(
        num_rows, num_real, budget, survival_eps):
    if num_real > num_rows:
...
def test_zero_epsilon_declines_fusion_and_preserves_tensor_semantics(monkeypatch):
    ...
    monkeypatch.setattr(
        device_select, "schedule_verify_lens_topk_fused_fill", _must_not_run)
...
def test_fused_scheduler_config_is_default_off_and_independently_gated():
    from tensorrt_llm.llmapi.llm_args import DSparkDecodingConfig

    default = DSparkDecodingConfig(max_draft_len=5)
    assert default.enable_confidence_scheduling is False
    assert default.enable_fused_confidence_scheduler is False

    with pytest.raises(ValueError, match="enable_fused_confidence_scheduler"):
        DSparkDecodingConfig(
            max_draft_len=5,
            enable_fused_confidence_scheduler=True,
        )

    enabled = DSparkDecodingConfig(
        max_draft_len=5,
    ...
    assert enabled.enable_fused_confidence_scheduler is True
...
def test_model_engine_prewarm_records_only_successful_graph_sizes(monkeypatch):
    ...
    monkeypatch.setattr(
        dspark_schedule, "schedule_verify_lens_topk_fused_fill", _fake_fused)
    PyTorchModelEngine._warmup_dspark_fused_scheduler(engine)
    assert calls == [16, 32]
